  ...
  return <div className="fixed right-0 top-0 bottom-0 w-96 shadow-2xl z-50 animate-slide-in-right border-l border-gray-200 flex flex-col bg-slate-200">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 min-h-0 bg-slate-200">
        ...
        {isInFlow && currentStep && currentStep.userOptions && currentStep.userOptions.length > 0 && <div className="space-y-2">
            {currentStep.userOptions.map((option, index) => <Button key={index} onClick={() => onFlowChoice(option.text, option.nextStep)} className="w-full bg-blue-600 hover:bg-blue-700 text-white border-0 rounded-md px-4 py-2 text-sm font-medium">
                {option.text}
              </Button>)}
          </div>}
      </div>
      ...
      {!hasOnlyButtonOptions && <div className="p-4 border-t border-gray-200 flex-shrink-0 bg-slate-500">
          <div className="relative">
            ...
            <Button onClick={sendMessage} size="sm" className="absolute right-2 top-1/2 transform -translate-y-1/2 bg-blue-600 hover:bg-blue-700 text-white border-0 h-8 w-8 p-0" disabled={!inputValue.trim()}>
