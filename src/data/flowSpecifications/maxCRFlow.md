...
# Max CR Troubleshooting Flow Specification
...
User can choose:
- "My Max CR turns on, but the charger will not turn on or the batteries do not hold a charge." -> go to: Step_For_Max_CR_battery_troubleshooting
...
- "No power" -> go to: replace_battery
...
- "No LEDs illuminate on the battery charger or just the red LED is flashing" -> go to: remove_fuse_check_continuity
...
Bot says:
- "Inspect all the wiring running between the receiver board and the breakout board. If the wiring is good, first replace the 12344-receiver board, if that doesn't resolve the problem replace the 13059-breakout board."
